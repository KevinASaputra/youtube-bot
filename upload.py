    ...
    for domain, cookies in domain_cookies.items():
        ...
        for cookie in cookies:
            ...
            try:
                ...
                fungsi_tulis(
                    f"Couldn't set cookie {cookie['name']} for {domain}")
...
def _wait_for_processing(driver):
    progress_label: WebElement = driver.find_element_by_css_selector(
        "span.progress-label")
    pattern = re.compile(
        r"(finished processing)|(Checks complete.*)|(processing hd.*)|(check.*)|(upload complete.*)")
...
def upload_file(
    ...
    WebDriverWait(driver, 20).until(EC.element_to_be_clickable(
        (By.CSS_SELECTOR, "ytcp-icon-button#upload-icon"))).click()
...
# #####setting deskripsi#####
    ...
    WebDriverWait(driver, 100).until(EC.element_to_be_clickable(
        (By.XPATH, '//ytcp-social-suggestions-textbox[@label="Description"]//div[@id="textbox"]')))
    ...
    WebDriverWait(driver, 50).until(EC.element_to_be_clickable(
        (By.NAME, 'VIDEO_MADE_FOR_KIDS_NOT_MFK'))).click()
    ...
    if STATUS:
        ...
        upload_time = upload_time+timedelta(days=random.randrange(
            1, int(30)), hours=random.randrange(1, 23), minutes=random.choice(menit))
        ...
        WebDriverWait(driver, 20).until(EC.element_to_be_clickable(
            (By.CSS_SELECTOR, "#datepicker-trigger > ytcp-dropdown-trigger:nth-child(1)"))).click()

        date_input: WebElement = driver.find_element_by_xpath(
            "//ytcp-date-picker//iron-input/input[@class='style-scope tp-yt-paper-input']")
        sleep(5)
        date_input.clear()
        ...
        WebDriverWait(driver, 20).until(EC.element_to_be_clickable(
            (By.XPATH, "//ytcp-form-input-container[@id='time-of-day-container']//iron-input/input[@class='style-scope tp-yt-paper-input']"))).click()
        ...
        WebDriverWait(driver, 50).until(EC.element_to_be_clickable(
            (By.XPATH, "//tp-yt-paper-radio-button[@class='style-scope ytcp-video-visibility-select'][3]"))).click()
    ...
    url_video_upload = WebDriverWait(driver, 100).until(EC.element_to_be_clickable(
        (By.CLASS_NAME, "video-url-fadeable"))).get_attribute("innerText")
    ...
    fungsi_tulis(
        f"Berhasil upload video {title} dengan URL {url_video_upload}")
...
for line in a:
    ...
    try:
        ...
        driver.quit()
        fungsi_tulis(
            "Upload Selesai video ke --> {} | {}".format(urutan, judul))
